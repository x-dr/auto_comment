...
spark_config_on = environ.get('SPARK_CONFIG_ON', 'false').lower() == 'true'
if spark_config_on:
    spark_appid = environ.get('SPARK_APPID')
    spark_key = environ.get('SPARK_KEY')
    spark_secret = environ.get('SPARK_SECRET')
...
def generation(pname, _class=0, _type=1, opts=None):
    ...
    if spark_config_on:
        spark_api = SparkApi(spark_appid, spark_key,spark_secret, spark_url, domain)
        msg_json = spark_api.get_answer(
        [{"role": "user", "content": f"写一段关于“{pname}”商品的评价，含有emoji表情，简短、口语化"}])
        if msg_json['code'] == 1:
            msg = msg_json['data']
            spark_api.answer = ""
            print(msg)
        ...
        for i, Order in enumerate(reversed(Order_data)):
            ...
            for oname, pid in zip(oname_data, pid_data):
                ...
                if imgdata["imgComments"]["imgCommentCount"] > 10:
                    pnum = random.randint(
                        2, int(imgdata["imgComments"]["imgCommentCount"]/10)+1)
                    opts['logger'].debug(
                        'Count of fetched image comments is 0')
                ...
                try:
                    imgurl = random.choice(imgdata["imgComments"]["imgList"])[
                        "imageUrl"]
                    if ('imgdata2' in dir()):
                        imgurl2 = random.choice(imgdata2["imgComments"]["imgList"])[
                            "imageUrl"]
                    else:
                        imgurl2 = ''
                except Exception:
                ...
                opts['logger'].debug(
                    'New header for this request: %s', headers)
                data = {
                    'orderId': oid,
                    'productId': pid,
                    'score': str(xing),  # 商品几星
                ...
                opts['logger'].debug(
                    'Sleep time (s): %.1f', ORDINARY_SLEEP_SEC)
        ...
        for i, Order in enumerate(reversed(Order_data)):
            ...
            if not opts.get('dry_run'):
                ...
                opts['logger'].debug(
                    'Skipped sending comment request in dry run')
            # opts['logger'].info("\n " + pj1.text)
            opts['logger'].debug('Sleep time (s): %.1f',
...
if __name__ == '__main__':
    ...
    formatter = StyleFormatter(
        '%(asctime)s %(levelname)-19s %(message)s', "%F %T")
   ...
   # with open(_cfg_path, 'r', encoding='utf-8') as f:
    ...
    try:
        i = 1
        cks = []
        if "PC_COOKIE" in environ:
            if len(environ["PC_COOKIE"]) > 200:
                if '&' in environ["PC_COOKIE"]:
                    cks = environ["PC_COOKIE"].split('&')
                else:
                    cks.append(environ["PC_COOKIE"])
            else:
